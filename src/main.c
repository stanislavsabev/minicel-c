...
String *read_csv(Arena *arena, const char *file_name) {
    ...
    usize read = fread(content->data, sizeof(char), content->len, fp);
    if (ferror(fp)) {
        goto error;
    }
...
error:
    ...
    if (content) {
        arena_snapshot_restore(snapshot);
    }
...
int cell_strview(
    const Cell *cell, const Expr *exprs, const bool with_type, char *buff, usize buff_len) {
...
i32 main(i32 argc, char const *argv[argc]) {
    ...
    // TODO: implement strcmp for String and StrView
    if (strncmp("-h", argv[1], strlen("-h")) == 0) {
        print_usage();
        return EXIT_SUCCESS;
    }

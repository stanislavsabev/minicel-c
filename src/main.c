...
void print_horizontal_line(usize cell_width, usize ncols) {
  usize repetitions = cell_width * ncols + ncols + 1;
...
Cell create_cell(usize row, usize col, usize row_count, Table *tbl) {
  ...
  if (row == 0) {
    ...
    Expr expr = {
        .str = expr_str, .eval_state = EVAL_STATE_NOT_STARTED, .value = 0};
...
String read_csv(Arena *a, const char *file_name) {
  ...
  String *result = (String *)arena_alloc(a, sizeof(String));

  buff = (char *)arena_alloc(a, len);
  if (buff == NULL) {
    goto error;
  }

  usize read = fread(buff, sizeof(char), len, fp);
  if (ferror(fp)) {
    goto error;
  }
...
int build_table(Table *tbl, StrView csv) {
  arr_init(Cell, cells);
  arr_init(Expr, exprs);
  StrView line = str_lsplit_chr(&csv, '\n');
...
int cell_strview(const Cell *cell, const Expr *exprs, const bool with_type,
                 char *buff, usize buff_len) {
  ...
  case VALUE_TYPE_NUMBER: {
    snprintf(buff, buff_len, with_type ? "NUMBER(%.2f)" : "%.2f",
             cell->value_as.number);
  } break;
  case VALUE_TYPE_TEXT:
    snprintf(buff, buff_len, with_type ? "TEXT(%s)" : "%s", cell->value_as.str);
    break;
  case VALUE_TYPE_ERR:
    snprintf(buff, buff_len, with_type ? "ERR(%s)" : "%s",
             cell->value_as.err_str);
    break;
  case VALUE_TYPE_EXPR: {
    usize index = cell->value_as.expr_ndx;
...
void print_table(Table *tbl) {
  ...
  const usize cell_width = CELL_PRINT_WIDTH / (with_type ? 1 : 2);

  print_horizontal_line(cell_width, tbl->ncols);
  for (usize r = 0; r < tbl->nrows; r++) {
    fprintf(stdout, "|");
...
#define EOF_TOKEN                                                              \
  (Token) { .type = TOKEN_TYPE_EOF, .s = NULL, .len = 0 }
...
Token operator_token(const char *s, usize len) {
  return (Token){.s = s, .len = 1, .type = TOKEN_TYPE_OPERATOR};
}

Token cell_address_token(const char *s, usize len) {
  Token token = {0};
  usize i = 0;
  ...
  if (!isdigit(s[i])) {
    fprintf(stderr,
            "Invalid cell address token. Expected row number at pos %lu", i);
    exit(EXIT_FAILURE);
  }
  while (isdigit(s[i])) {
    ++i;
...
Token get_next_token(const char **s, usize *len) {
  // number
  usize ln = *len;
  const char *sp = *s;
...
void consume(const char *what, const char **s, usize *len) {
  const char *sp = *s;
  usize i = 0;
...
AddressRC cell_address_rc(const char *s, usize len) {
  usize col = 0;
  usize row = 0;
  usize i = 0;
  ...
  if (MAX_COLUMNS < col) {
    fprintf(stderr, "Invalid column address %.*s. Max column address is 'ZZ'",
            (int)i, s);
    exit(EXIT_FAILURE);
  }
  ...
  if (MAX_ROWS < row) {
    fprintf(stderr, "Invalid row address %.*s. Max row address is 'ZZ'", (int)i,
            s);
    exit(EXIT_FAILURE);
  }
  ...
  while (ln) {
    ...
    case TOKEN_TYPE_NUMBER: {
      ...
      dfprintf(stderr, "lookahead: %.*s, type: %s\n", (int)lookahead.len,
               lookahead.s, TokenType_to_str(lookahead.type));
...
void evaluate_expr_cell(Cell *c, Table *tbl) {
  ...
  f64 result = evaluate_ast(&ast, tbl);
  expr->value = result;
...
f64 evaluate_ast(Ast *ast, Table *tbl) {
  ...
  case AST_VALUE_TYPE_BINARY: {
    f64 lhs_res = evaluate_ast(&ast->val_as->binary.lhs, tbl);
    f64 rhs_res = evaluate_ast(&ast->val_as->binary.rhs, tbl);
  ...
  case AST_VALUE_TYPE_ADDRESS_RC: {
    ...
    if (c->value_type == VALUE_TYPE_EXPR) {
      ...
      fprintf(stderr, "Non numeric cell(%lu, %lu) cannot be evaluated", c->row,
              c->col);
    }
  }
  default:
...
void print_usage() { puts("usage: ./minicell <filename.csv>"); }
